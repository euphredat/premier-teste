...
function setupEventListeners() {
  ...
  // Gérer les changements de couche de carte
  const layerRadios = document.querySelectorAll("#layer-menu input[name='layer']");
  layerRadios.forEach((radio) => {
    radio.addEventListener("change", (event) => {
      setActiveMapLayer(event.target.value);
    });
  });

  // Ajouter un gestionnaire pour l'icône flottante
  const layerToggleIcon = document.getElementById("layer-toggle-icon");
  if (layerToggleIcon) {
    layerToggleIcon.addEventListener("click", toggleLayer);
  }
}
...
function setActiveMapLayer(layerName) {
  ...
  switch (layerName) {
    case "standard":
      ...
      break;
    case "satellite":
      buttonId = "satellite-btn";
      break;
    case "aerial":
      buttonId = "aerial-btn";
      break;
    case "terrain":
      buttonId = "terrain-btn";
      break;
  }
...
async function loadGpxFile(filePath, isReversed) {
  try {
    ...
    // Si l'itinéraire doit être inversé, inverser les coordonnées
    if (isReversed) {
      const parser = new DOMParser();
      const xmlDoc = parser.parseFromString(gpxData, "text/xml");
      const trackPoints = Array.from(xmlDoc.getElementsByTagName("trkpt")).reverse();
      const reversedCoordinates = trackPoints.map((trkpt) => [
